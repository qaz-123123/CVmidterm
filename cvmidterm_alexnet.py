...
def train_model(device, lr):
    # optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, alexnet.parameters()), lr=lr)
...
# optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, alexnet.parameters()), lr)

for epoch in range(epochs_frozen):
    lr = lr * decay
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, alexnet.parameters()), lr)
...
# base_params = filter(lambda p: id(p) not in alexnet.classifier[6].parameters(), alexnet.parameters())
base_params = [p for p in alexnet.parameters() if id(p) not in [id(param) for param in alexnet.classifier[6].parameters()]]
optimizer = torch.optim.SGD([
        {'params': base_params},
        {'params': alexnet.classifier[6].parameters(), 'lr': pow(10, -5)}
    ], lr=pow(10, -4))
...
plt.savefig('figure//alexnet//epochfro20&epochunfro200&SGD&stratifiedlr&decay09.png', dpi=300)
plt.show()

torch.save(alexnet, 'save_models//trained_alexnet//epochfro20&epochunfro200&SGD&stratifiedlr&decay09.pth')
